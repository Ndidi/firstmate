...
config/crew-memory-max  per-worker memory bound applied to every spawn, covering the agent and every process it launches; LOCAL, gitignored; absent = a quarter of host memory clamped to [2G, 8G], a size such as "6G" overrides it, "off" disables it; inherited by secondmate homes; a host that cannot apply the bound spawns unbounded and says so (docs/configuration.md "Crew memory cap")
...
config/pool-reserve  optional warm-copy reserve and settling window for the isolated-copy pool; LOCAL, gitignored; absent means the built-in defaults, which need no configuration; inherited by secondmate homes (docs/configuration.md "Worktree pool reserve")
...
Load `requirement-elicitation` before writing a brief for work the captain described in a sentence or two, where the acceptance criteria, constraints, and scope would otherwise come from firstmate's own inference; it does not fire for a request that already carries its scope, a bug fix, a mechanical change, a settled follow-up, or anything the captain has already specified.
...
Cleanup also sweeps the isolated-copy pool back to its warm reserve through `bin/fm-pool-reap.sh`, which refuses any copy holding unlanded, claimed, or unverifiable work; treat its refusals as reported results rather than failures, and relay a copy it reports as permanently stuck to the captain, because only they can authorize discarding it.
...
- `requirement-elicitation` - load before writing a brief for work described in a sentence or two whose acceptance criteria, constraints, and scope firstmate would otherwise infer; err toward not firing.
- `investigation-framing` - load before scaffolding any scout brief, and when confirming an investigation's framing with the captain.
