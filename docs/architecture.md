...
A busy pane past that bound whose crew declares an external wait takes the longer `FM_PAUSE_RESURFACE_SECS` cadence instead of that escalation, because a declared wait is otherwise unreachable while the pane is busy.
