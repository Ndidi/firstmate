...
family_for_basename() {
  case "$1" in
    ...
    fm-scout-framing.test.sh|fm-send-popup-settle.test.sh|fm-send-settle.test.sh|\
...
run_coverage_guard() {
  ...
  # Every listing above is sorted under LC_ALL=C, so every comparison below must
  # read it the same way. Ambient collation ignores punctuation that C orders by
  # byte, so an unprefixed comm rejects its own sorted input ("file 2 is not in
  # sorted order") and the guard fails for everyone outside a C locale.
